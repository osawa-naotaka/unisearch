import { difference, intersect, zipWith } from "@ref/algo";
...
export async function execBenchmark<T>(
    ...
    const result_search = benchmark(
        (key) => ({ keyword: key, refs: searcher.search_fn(key, searcher.index) }),
        keywords,
    );
...
async function prepareAndExecBenchmark<T>(
    ...
    console.log(
        cheked_results
            .map((r) => ({ keyword: r.keyword, false_negative: r.false_negative }))
            .filter((x) => x.false_negative.length !== 0),
    );
...
export function generateBenchmarkRunner(
    ...
    return <T>(name: string, searcher: SearcherSet<T>) =>
        prepareAndExecBenchmark(name, articles, keywords, ref_results, searcher);
}
