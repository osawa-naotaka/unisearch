...
@compute @workgroup_size(256) fn cs(
    ...
    const vec_zero   = vec3u(0u, 0u, 0u);
    const vec_one    = vec3u(1u, 1u, 1u);
    let end_mask_v = vec3u(end_mask, end_mask, end_mask);
    var state = vec_zero;

    let end_pos = id.x + keyword_len + 2; // 2 means distance 2
    var dist = 4u;
    var tail_pos = 0u;
    for(var pos = id.x; pos < end_pos; pos++) {
        ...
        let next_state_candidate_mask = next_state_candidate & vec3u(mask, mask, mask);
        ...
        if(any(matches)) {
            let d = select(select(2u, 1u, matches[1]), 0u, matches[0]);
            if(d < dist) {
                tail_pos = pos;
                dist = d;
            }
