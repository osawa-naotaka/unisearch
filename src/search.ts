import type { SearchIndex, SearchResult, UniIndex } from "@src/base";
...
export function search<T>(index: UniIndex<SearchIndex<T>>, query: string): SearchResult[] | UniSearchError {
    ...
    return ast
        ? evalQuery(index)(ast.val).sort((a, b) => b.score - a.score)
        : new UniSearchError("fail to parse query");
}

const evalQuery =
    <T>(index: UniIndex<SearchIndex<T>>) =>
    (ast: ASTNode): SearchResult[] => {
        switch (ast.type) {
            ...
            case "or":
                return unionResults(ast.nodes.map(evalQuery(index)));
            default:
                return [];
        }
    };
